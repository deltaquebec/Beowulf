...
def plot_language_families_stack(df):
    ...
    for index, row in df.iterrows():
        ...
        words = row['Unique Content'].split()
        # create an empty dictionary to store the count of each language family for this row
        row_language_counts = {'Germ.': 0, 'Lat.': 0, 'Hell.': 0, 'Sem.': 0, 'Celt.': 0, 'Indo-Iran.': 0, 'Turk.': 0, 'Ural.': 0}
        # iterate over the words
        for word in words:
            # classify the word's language family
            language_family = classify_language_family(word.lower())
            # if the language family is not "Other", increment the count for this language family in the row's dictionary
    ...
    for row, language_count_dict in language_family_counts.items():
        ...
        ax.bar(row, counts[0], color='#009E73')
        ax.bar(row, counts[1], bottom=counts[0], color='#E69F00')
        ax.bar(row, counts[2], bottom=sum(counts[:2]), color='#0072B2')
        ax.bar(row, counts[3], bottom=sum(counts[:3]), color='#D55E00')
        ax.bar(row, counts[4], bottom=sum(counts[:4]), color='#CC79A7')
        ax.bar(row, counts[5], bottom=sum(counts[:5]), color='#56B4E9')
        ax.bar(row, counts[6], bottom=sum(counts[:6]), color='#F0E442')
        ax.bar(row, counts[7], bottom=sum(counts[:7]), color='#CC99CC')
    ...
    # create a legend
    handles = [plt.Rectangle((0,0),1,1,color=c,ec="k") for c in ['#009E73', '#E69F00', '#0072B2', '#D55E00', '#CC79A7', '#56B4E9', '#F0E442', '#CC99CC']]
    labels = ['Germ.', 'Lat.', 'Hell.', 'Sem.', 'Celt.', 'Indo-Iran.', 'Turk.', 'Ural.']
...
def plot_language_families(df):
    ...
    language_families = ['Germ.', 'Lat.', 'Hell.', 'Sem.', 'Celt.', 'Indo-Iran.', 'Turk.', 'Ural.']

    # create a list of colors to use for the bars
    colors = ['#009E73', '#E69F00', '#0072B2', '#D55E00', '#CC79A7', '#56B4E9', '#F0E442', '#CC99CC']

    # iterate over the rows in the language family counts dictionary
    for row, language_count_dict in language_family_counts.items():
        # extract the counts for each language family for this row
        counts = [language_count_dict[language_family] for language_family in language_families]
...
df = pd.read_csv("df_clean_hall.csv")
