...
def raw_df(directory):
    ...
    for file in text_files:
        ...
        temp_df = pd.DataFrame({'Filename': [file], 'Number of Words': [num_words], 'Number of Unique Words': [num_unique_words],
                                'Average Word Length': [avg_word_length], 'Average Sentence Length': [avg_sentence_length],
                                'Content': [text], 'Average Words per Sentence': [avg_words_per_sentence], 'Lexical Density': [lexical_density]})
        df_raw = pd.concat([df_raw, temp_df], ignore_index=True)
    return df_raw

# test the function
df_raw = raw_df(r"C:\Users\delta\AppData\Local\Programs\Python\Python37\hw\project\oe")
# Save the dataframe to a CSV file
df_raw.to_csv('df_raw_oe.csv')

print(df_raw)
print(df_raw[['Number of Words', 'Number of Unique Words','Average Word Length','Average Sentence Length','Lexical Density']])
...
def clean_df(directory):
    ...
    for file in text_files:
        ...
        with open(os.path.join(directory, file), 'r', encoding=result["encoding"]) as f:
            ...
            # stopwords
            with open('oe_stopwords.txt', "rb") as input_file:
                data = input_file.read()
                result = chardet.detect(data)
            with open('oe_stopwords.txt','r', encoding=result["encoding"]) as f:
                newStopWords = f.readlines()
            stop_words.update([word.strip() for word in newStopWords])
 ...
 # test the function
df_clean = clean_df(r"C:\Users\delta\AppData\Local\Programs\Python\Python37\hw\project\oe")
# Save the dataframe to a CSV file
df_clean.to_csv('df_clean_oe.csv')

print(df_clean)
print(df_clean[['Number of Words', 'Number of Unique Words','Average Word Length','Average Sentence Length','Lexical Density']])
